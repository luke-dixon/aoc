...
class Instruction:
    def __init__(self, data, position, operation, input_, output, parameter1, parameter2, parameter3):
        self.data = data
        self.position = position
...
def instruction_factory(data, position, input_, output):
...
class Day5(Puzzle):
    def __init__(self):
        ...
        self.data = tuple([int(x) for x in self.input_data.split(',')])

    def part1(self):
        output = run_intcode_computer(list(self.data), [1])
        return output[-1]

    def part2(self):
        output = run_intcode_computer(list(self.data), [5])
        return output[-1]
